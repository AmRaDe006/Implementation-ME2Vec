...
def parse_args():
    ...
    parser.add_argument('--train_ratio', type=float, default=0.8,
                        help='train ratio manually addded as it was missing')
    
    parser.add_argument('--drop_enc', type=int, default=1,
                        help='drop_enc manually added as it was missing')
...
def main(args):
    ...
    adj_mat = np.zeros((num_doc, num_svc), dtype=int)
    for i in range(num_doc):
        adj_mat[i, doc_svc[i]] = 1 # type: ignore

    doc_emb_train = doc_emb[:round(num_doc * args.train_ratio)]
    doc_emb_test = doc_emb[round(num_doc * args.train_ratio):]

    doc_spec_train = doc_spec[:round(num_doc * args.train_ratio)]# type: ignore
    doc_spec_test = doc_spec[round(num_doc * args.train_ratio):]# type: ignore
